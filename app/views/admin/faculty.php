...
<div class="container mt-5">
    ...
    <div class="card shadow mb-4">
        <div class="card-body">
            <form method="GET" action="<?= BASE_URL ?>/admin/faculty" class="row g-3 align-items-center">
                <div class="col-md-4">
                ...
                <?php if (!empty($search)): ?>
                    <div class="col-md-2">
                        <a href="<?= BASE_URL ?>/admin/faculty" class="btn btn-outline-secondary w-100">Clear Search</a>
                    </div>
                <?php endif; ?>
            </form>
        </div>
    ...
    <div class="card shadow">
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        <?php if (empty($users)): ?>
                            <tr>
                                <td colspan="5" class="text-center py-4 text-muted">No faculty members found.</td>
                            </tr>
                        <?php else: ?>
                                ...
                                <tr>
                                    ...
                                    <td>
                                       <div class="btn-group btn-group-sm">
                                        ...
                                        <?php if ($user['is_active']): ?>
                                            <button class="btn btn-warning btn-sm mb-3 confirm-action-btn" 
                                                    data-action="block"
                                                    data-url="<?= BASE_URL ?>/admin/toggleStatus/<?= $user['id'] ?>?status=0"
                                                    data-title="Block Faculty"
                                                    data-message="Are you sure you want to deactivate '<?= htmlspecialchars($user['name']) ?>'?"
                                        ...
                                        <button class="btn btn-danger btn-sm mb-3 confirm-action-btn" 
                                                data-action="delete"
                                                data-url="<?= BASE_URL ?>/admin/deleteUser/<?= $user['id'] ?>"
                                                data-title="Delete Faculty"
                                                data-message="Are you sure you want to permanently delete '<?= htmlspecialchars($user['name']) ?>'? This action cannot be undone."
                                                data-bs-toggle="modal" 
                                                data-bs-target="#confirmActionModal">
            ...
            <?php if ($totalPages > 1): ?>
                <?php 
                $baseUrl = BASE_URL . "/admin/faculty?limit=" . $limit . "&search=" . urlencode($search);
                ?>
                <nav aria-label="Faculty navigation" class="mt-4">
    ...
    confirmButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (action === 'delete') {
                ...
                confirmBtn.innerHTML = '<i class="fas fa-ban"></i> Block';
            } else {
